...
class StaticUniSimulator(BasicSimulator):
    def initialize(self):
        self.client_time_response = {cid: self.random_module.randint(5, 1000) for cid in self.clients}
...
class DynamicUniSimulator(BasicSimulator):
    def initialize(self):
        client_time_response = {cid: self.random_module.randint(5, 1000) for cid in self.clients}
        self.set_variable(list(self.clients.keys()), 'latency', list(client_time_response.values()))

    def update_client_responsiveness(self, client_ids):
        client_time_response = {cid: self.random_module.randint(5, 1000) for cid in client_ids}
